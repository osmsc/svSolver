...
!     Uc = D*Um
      CALL svLS_SPARMULVS(lhs, lhs%rowPtr, lhs%colPtr, nsd, mD, Um, Uc)
!     Uc = Xc - Uc
      Uc = Xc - Uc                                   
!     Uc = [L + G^t*G]^-1*Uc
      CALL CGRAD_SCHUR(lhs, ls%CG, nsd, Gt, mG, mL, Uc)
!     Um = G*Uc
      CALL svLS_SPARMULSV(lhs, lhs%rowPtr, lhs%colPtr, nsd, mG, Uc, Um)
!     Xm = Xm - Um
      Xm = Xm - Um
!     Um = K^-1*Xm
      CALL GMRES(lhs, ls%GM, nsd, mK, Xm, Um)        
      ...
      CALL svLS_SPARMULVV(lhs, lhs%rowPtr, lhs%colPtr, dof, Val, Ua, R)
