...
!     These are the default values
      ...
      IF (.NOT.resetSim) THEN   
         ...
         IF (ltmp) appPath = STR(cm%np())//"-procs"//delimiter
         ...
         std = "                                              "
         std = " ----------------------------------------------"
         std = "             Copyright (C), 2013               "
         std = "            Mahdi Esmaily Moghadam             "
         std = "                                               "
         std = "      Multi-Physics Finite Element Solver      "
         std = "                  (MUPFES v"//version//") "
         std = " ----------------------------------------------"
         std = "                                               "
     ...
     2      "Increment in saving restart files",ll=1)
...
!     FLUID Navier-Stokes solver ------------------------------------
      CASE ('fluid')
         ...
         CALL READLS(LS_TYPE_NS, lEq%ls, list)
!     HEAT FLUID advection diffusion solver -------------------------
      CASE ('heatF')
         ...
         CALL READLS(LS_TYPE_GMRES, lEq%ls, list)
...
!     LINEAR ELASTICITY equation solver------------------------------
      CASE ('lElas')
         ...
         CALL READLS(LS_TYPE_CG, lEq%ls, list)
...
!     This subroutine reads LS type, tolerance, ...
      SUBROUTINE READLS(ilsType, ls, list)
      USE COMMOD
      USE LISTMOD
      USE ALLFUN
      IMPLICIT NONE
