...
!$OMP END PARALLEL
      ...
      IF (ISZERO(eq(cEq)%ls%RI%iNorm)) eq(cEq)%ls%RI%iNorm = eps
      IF (ISZERO(eq(cEq)%iNorm)) eq(cEq)%iNorm = eq(cEq)%ls%RI%iNorm
      IF (eq(cEq)%itr .EQ. 1) THEN
         eq(cEq)%pNorm = eq(cEq)%ls%RI%iNorm/eq(cEq)%iNorm
      END IF
      tmp = 2D1*LOG10(eq(cEq)%ls%RI%iNorm/eq(cEq)%iNorm/eq(cEq)%pNorm)
      
      l1 = eq(cEq)%itr .GE. eq(cEq)%maxItr
      l2 = eq(cEq)%ls%RI%iNorm .LE. eq(cEq)%tol*eq(cEq)%iNorm
